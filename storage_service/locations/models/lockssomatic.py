...
class Lockssomatic(models.Model):
    ...
    au_size = models.BigIntegerField(
        verbose_name=_("AU Size"),
        null=True,
        blank=True,
        help_text=_("Size in bytes of an Allocation Unit"),
    )
    sd_iri = models.URLField(
        max_length=256,
        verbose_name=_("Service Document IRI"),
        help_text=_(
            "URL of LOCKSS-o-matic service document IRI, eg. http://lockssomatic.example.org/api/sword/2.0/sd-iri"
        ),
    )
    collection_iri = models.CharField(
        max_length=256,
        null=True,
        blank=True,
        verbose_name=_("Collection IRI"),
        help_text=_(
            "URL to post the packages to, eg. http://lockssomatic.example.org/api/sword/2.0/col-iri/12"
        ),
    )
    content_provider_id = models.CharField(
        max_length=32,
        verbose_name=_("Content Provider ID"),
        help_text=_("On-Behalf-Of value when communicating with LOCKSS-o-matic"),
    )
    external_domain = models.URLField(
        verbose_name=_("Externally available domain"),
        help_text=_(
            "Base URL for this server that LOCKSS will be able to access.  Probably the URL for the home page of the Storage Service."
        ),
    )
    checksum_type = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Checksum type"),
        help_text=_(
            "Checksum type to send to LOCKSS-o-matic for verification.  Eg. md5, sha1, sha256"
        ),
    )
    keep_local = models.BooleanField(
        blank=True,
        default=True,
        verbose_name=_("Keep local copy?"),
        help_text=_(
            "If checked, keep a local copy even after the AIP is stored in the LOCKSS network."
        ),
    )

    class Meta:
        verbose_name = _("LOCKSS-o-matic")
        app_label = "locations"
    ...
    def post_move_from_storage_service(self, staging_path, destination_path, package):
        ...
        receipt = self.sword_connection.create(
            col_iri=self.collection_iri, metadata_entry=entry, suggested_identifier=slug
        )
        try:
    ...
    def update_package_status(self, package):
        ...
        if (
            "state_iri" not in package.misc_attributes
            or "edit_iri" not in package.misc_attributes
        ):
        ...
        for index, file_e in enumerate(files):
            ...
            for old_url in file_e.findall(
                "mets:FLocat[@LOCTYPE='URL']", namespaces=utils.NSMAP
            ):
            ...
            for server in lom_servers:
                ...
                flocat = etree.SubElement(
                    file_e, utils.PREFIX_NS["mets"] + "FLocat", LOCTYPE="URL"
                )
    ...
    def _delete_update_lom(self, package, delete_lom_ids):
        ...
        for lom_id in delete_lom_ids:
            if lom_id:
                etree.SubElement(
                    entry.entry, utils.PREFIX_NS["lom"] + "content", recrawl="false"
                ).text = lom_id
    ...
    def _delete_files(self):
        ...
        if not self.keep_local and delete_elements:
            ...
            # If file was split
            if (
                self.pointer_root.find(
                    ".//mets:fileGrp[@USE='LOCKSS chunk']", namespaces=utils.NSMAP
                )
                is not None
            ):
    ...
    def update_service_document(self):
        ...
        try:
            ...
            LOGGER.warning(
                "No collection IRI found in LOCKSS-o-matic service document."
            )
            return False

        # Checksum type - LOM specific tag
        root = self.sword_connection.sd.service_dom
        self.checksum_type = root.findtext(
            "lom:uploadChecksumType", namespaces=utils.NSMAP
        )
    ...
    def _split_package(self, package):
        ...
        if self.pointer_root.xpath(
            './/premis:eventType[text()="division"]', namespaces=utils.NSMAP
        ):
        ...
        self.pointer_root.find("mets:structMap", namespaces=utils.NSMAP).set(
            "TYPE", "logical"
        )
        ...
        filegrp = etree.SubElement(
            filesec, utils.PREFIX_NS["mets"] + "fileGrp", USE="LOCKSS chunk"
        )
        ...
        if local_ftpr is not None:
            div = etree.SubElement(
                aip_div, utils.PREFIX_NS["mets"] + "div", TYPE="Local copy"
            )
        ...
        for idx, out_path in enumerate(output_files):
            ...
            etree.SubElement(
                div, utils.PREFIX_NS["mets"] + "fptr", FILEID=os.path.basename(out_path)
            )
    ...
    def _create_resource(self, package, output_files):
        ...
        if dublincore is not None:
            title = dublincore.findtext(
                "dcterms:title", namespaces=utils.NSMAP, default=title
            )
            slug = dublincore.findtext(
                "dcterms:title", namespaces=utils.NSMAP, default=slug
            )
            summary = dublincore.findtext(
                "dcterms:description", namespaces=utils.NSMAP, default=summary
            )
